...
def iter_norm_mod(X, running_mean=None, running_cov=None, T=5, eps=1e-5, momentum=0.1,cov_warmup=False):
    # Use is_grad_enabled to determine whether we are in training mode
    assert len(X.shape) in (2, 4)
    n_features = X.shape[1]
...
class BWItnBlock(nn.Module):
    ...
    # number of output channels for a convolutional layer. num_dims: 2 for a
    # fully connected layer and 4 for a convolutional layer
    def __init__(self, num_features,T=5,momentum=0.1,eps=1e-5,pre_bias_block=None,num_bias_features=None):
        super().__init__()
        # The scale parameter and the shift parameter (model parameters) are
        # initialized to 1 and 0, respectively 
        self.T=T
    ...
    def forward(self, X):
        ...
        # Save the updated running_mean and moving_var
        Y, self.running_mean, self.running_cov = batch_orthonorm(
            X, self.running_mean, self.running_cov, T=self.T, eps=self.eps, momentum=self.momentum,cov_warmup=self.cov_warmup)
        if self.pre_bias_block is not None:
            Y=self.pre_bias_block(Y)
        # add the bias
        shape = (1,self.n_bias_features) if len(X.shape)==2 else (1,self.n_bias_features,1,1)
